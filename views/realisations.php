...
function uploadImage($file, $prefix = 'real')
{
  ...
  if (move_uploaded_file($file['tmp_name'], $destination)) {
    return ['success' => true, 'path' => 'uploads/realisations/' . $filename];
  }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajouter_realisation'])) {
  try {
    ...
    $stmt = $pdo->prepare("INSERT INTO realisations (boutique_id, titre, description, image_principale, categorie, date_realisation, client_nom, client_ville) VALUES (?,?,?,?,?,?,?,?)");
    $stmt->execute([$bid, $titre, $description, $image_principale, $categorie, $date_realisation, $client_nom, $client_ville]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['modifier_realisation'])) {
  try {
    ...
    if (isset($_FILES['image_principale']) && $_FILES['image_principale']['error'] === UPLOAD_ERR_OK) {
      ...
      if (isset($upload_main['error'])) throw new Exception($upload_main['error']);
      $image_principale = $upload_main['path'];
      $old_path = __DIR__ . '/../' . $old_main;
      if ($old_main && file_exists($old_path)) @unlink($old_path);
    }

    $pdo->prepare("UPDATE realisations SET titre=?, description=?, image_principale=?, categorie=?, date_realisation=?, client_nom=?, client_ville=? WHERE id=?")
      ->execute([$titre, $description, $image_principale, $categorie, $date_realisation, $client_nom, $client_ville, $real_id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['supprimer_realisation'])) {
  ...
  if (!$is_pdg) {
    ...
    $check->execute([$real_id, $boutique_id]);
    if (!$check->fetch()) {
      $_SESSION['flash_message'] = ['text' => "Non autorisé", 'type' => "error"];
      header("Location: realisations.php");
      exit;
    }
  }
...
<body class="h-screen flex overflow-hidden">
  ...
  <aside id="sidebar" class="sidebar w-64 flex flex-col fixed md:sticky top-0 h-full z-50 transition-transform duration-300 text-white">
    <div class="p-5 border-b border-white/10 flex-shrink-0">
      <div class="flex items-center gap-3">
        <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-pink-500 to-rose-600 flex items-center justify-center shadow-lg"><span class="font-bold text-white">NGS</span></div>
        <div>
    ...
    <div class="p-5 border-b border-white/10 flex-shrink-0">
      <div class="flex items-center gap-3">
        <div class="w-10 h-10 rounded-full bg-pink-500/20 border border-pink-400/30 flex items-center justify-center"><i class="fas fa-<?= $is_pdg ? 'crown text-amber-400' : 'store text-blue-400' ?>"></i></div>
        <div class="min-w-0">
    ...
    <header class="sticky top-0 z-30 glass border-b border-white/10">
      <div class="flex items-center justify-between px-4 md:px-6 py-4">
        <div class="flex items-center gap-3"><button id="mobileMenuBtn" class="md:hidden p-2 rounded-lg hover:bg-white/10 transition-colors text-[var(--text-primary)]"><i class="fas fa-bars text-lg"></i></button>
          <div>
            <h1 class="text-lg md:text-xl font-bold text-[var(--text-primary)]">Gestion des Réalisations</h1>
            <p class="text-xs text-[var(--text-muted)]">Portfolio & Demandes de service • <?= $total_realisations ?> réalisation(s)</p>
          </div>
        </div>
    ...
    <div class="p-4 md:p-6 space-y-6">
      <?php if ($message): ?>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div class="premium-card p-5 stat-card animate-fade-in-up border-l-4 border-pink-500" style="animation-delay:0s">
            <div class="flex items-center justify-between mb-2"><span class="text-xs font-medium text-pink-600 dark:text-pink-400">Total</span>
              <div class="w-8 h-8 rounded-lg bg-pink-100 dark:bg-pink-900/30 flex items-center justify-center"><i class="fas fa-images text-pink-600 dark:text-pink-400 text-sm"></i></div>
            </div>
            <p class="text-2xl font-bold text-[var(--text-primary)]"><?= $total_realisations ?></p>
            <p class="text-xs text-[var(--text-muted)] mt-1"><?= count(array_filter($realisations, fn($r) => $r['est_publie'])) ?> publiées</p>
          </div>
          <div class="premium-card p-5 stat-card animate-fade-in-up border-l-4 border-rose-500" style="animation-delay:0.1s">
            <div class="flex items-center justify-between mb-2"><span class="text-xs font-medium text-rose-600 dark:text-rose-400">Likes</span>
              <div class="w-8 h-8 rounded-lg bg-rose-100 dark:bg-rose-900/30 flex items-center justify-center"><i class="fas fa-heart text-rose-600 dark:text-rose-400 text-sm"></i></div>
            </div>
            <p class="text-2xl font-bold text-[var(--text-primary)]"><?= $total_likes ?></p>
          ...
          <div class="premium-card p-5 stat-card animate-fade-in-up border-l-4 border-amber-500" style="animation-delay:0.2s">
            <div class="flex items-center justify-between mb-2"><span class="text-xs font-medium text-amber-600 dark:text-amber-400">Demandes</span>
              <div class="w-8 h-8 rounded-lg bg-amber-100 dark:bg-amber-900/30 flex items-center justify-center"><i class="fas fa-envelope text-amber-600 dark:text-amber-400 text-sm"></i></div>
            </div>
            <p class="text-2xl font-bold text-[var(--text-primary)]"><?= $total_demandes ?></p>
          ...
          <div class="premium-card p-5 stat-card animate-fade-in-up border-l-4 border-emerald-500" style="animation-delay:0.3s">
            <div class="flex items-center justify-between mb-2"><span class="text-xs font-medium text-emerald-600 dark:text-emerald-400">Page</span>
              <div class="w-8 h-8 rounded-lg bg-emerald-100 dark:bg-emerald-900/30 flex items-center justify-center"><i class="fas fa-file-alt text-emerald-600 dark:text-emerald-400 text-sm"></i></div>
            </div>
          ...
          <div id="tab-realisations" class="tab-content space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <?php if (!empty($realisations)): ?>
                ...
                <?php endforeach; ?>
              <?php else: ?>
                <div class="col-span-full text-center py-12"><i class="fas fa-images text-5xl text-[var(--text-muted)] opacity-30 mb-4 block"></i>
                  <p class="text-[var(--text-secondary)] text-lg">Aucune réalisation</p>
                  <p class="text-xs text-[var(--text-muted)] mt-1">Ajoutez votre première réalisation</p>
                </div>
              <?php endif; ?>
            </div>
          </div>
          ...
          <div id="tab-demandes" class="tab-content hidden">
            <div class="premium-card overflow-hidden">
              <div class="px-5 py-4 border-b border-[var(--divider)] flex items-center justify-between">
                <h3 class="text-base font-bold text-[var(--text-primary)]">Demandes de service</h3><span class="text-xs text-[var(--text-muted)]"><?= $total_demandes ?> demande(s)</span>
              </div>
                ...
                <table class="w-full min-w-[800px]">
                  <thead>
                    <tr class="border-b border-[var(--divider)] text-left">
                      ...
                      <th class="px-5 py-3 text-xs font-semibold text-[var(--text-muted)] uppercase">Réalisation</th>
                  ...
                  <tbody class="divide-y divide-[var(--divider)]">
                    <?php if (!empty($demandes)): ?>
                      <?php foreach ($demandes as $d): $sc = ['nouvelle' => 'badge-warning', 'contactee' => 'badge-info', 'en_cours' => 'badge-info', 'terminee' => 'badge-success', 'annulee' => 'badge-danger']; ?>
                        <tr class="hover:bg-white/5 transition-colors">
                          ...
                          <td class="px-5 py-3.5">
                            <div class="flex items-center justify-center gap-1.5"><button onclick="openDemandeDetail(<?= $d['id'] ?>)" class="p-1.5 rounded-lg bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 hover:bg-blue-200 dark:hover:bg-blue-900/50 transition-colors" title="Détails"><i class="fas fa-eye text-xs"></i></button>
                      ...
                      <?php endforeach; ?>
                    <?php else: ?><tr>
                        <td colspan="7" class="px-5 py-12 text-center"><i class="fas fa-envelope text-4xl text-[var(--text-muted)] opacity-30 mb-3 block"></i>
                          <p class="text-[var(--text-secondary)]">Aucune demande</p>
                        </td>
                      </tr><?php endif; ?>
                  </tbody>
                </table>
  ...
  <div id="ajoutContent" class="modal-container fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[101] hidden w-[95%] max-w-lg p-6">
    <div class="flex items-center justify-between mb-5">
      <h3 class="text-lg font-bold text-[var(--text-primary)]"><i class="fas fa-plus-circle mr-2 text-pink-500"></i>Nouvelle réalisation</h3><button onclick="closeAjoutModal()" class="text-[var(--text-muted)] hover:text-[var(--text-primary)]"><i class="fas fa-times"></i></button>
    </div>
    <form method="POST" action="" enctype="multipart/form-data" class="space-y-4">
      ...
      <div class="grid grid-cols-2 gap-3">
        <div><label class="block text-xs font-medium text-[var(--text-secondary)] mb-1.5">Titre *</label><input type="text" name="titre" required class="w-full input-glass px-3 py-2.5 text-sm" placeholder="Ex: Rideaux salon luxueux"></div>
        <div><label class="block text-xs font-medium text-[var(--text-secondary)] mb-1.5">Catégorie</label><select name="categorie" class="w-full input-glass px-3 py-2.5 text-sm">
            <option value="rideaux">Rideaux</option>
            <option value="voilages">Voilages</option>
      ...
      <div><label class="block text-xs font-medium text-[var(--text-secondary)] mb-1.5">Image principale *</label>
        <div class="file-upload-area" id="mainUploadArea" onclick="document.getElementById('mainFile').click()"><input type="file" name="image_principale" id="mainFile" accept="image/*" required class="hidden" onchange="previewFile(this, 'mainPreview', 'mainUploadArea')">
          <div id="mainPreview" class="space-y-2"><i class="fas fa-cloud-upload-alt text-3xl text-[var(--text-muted)]"></i>
            <p class="text-sm text-[var(--text-muted)]">Cliquez pour sélectionner l'image principale</p>
            <p class="text-xs text-[var(--text-muted)]">JPG, PNG, WEBP - Max 10 MB</p>
          </div>
        </div>
      </div>
      <div class="grid grid-cols-2 gap-3">
        <div><label class="block text-xs font-medium text-[var(--text-secondary)] mb-1.5">Image secondaire 1</label>
          <div class="file-upload-area" id="supp1Area" onclick="document.getElementById('supp1File').click()"><input type="file" name="image_supp_1" id="supp1File" accept="image/*" class="hidden" onchange="previewFile(this, 'supp1Preview', 'supp1Area')">
            <div id="supp1Preview" class="space-y-1"><i class="fas fa-cloud-upload-alt text-2xl text-[var(--text-muted)]"></i>
              <p class="text-xs text-[var(--text-muted)]">Optionnelle</p>
            </div>
          </div>
        </div>
        <div><label class="block text-xs font-medium text-[var(--text-secondary)] mb-1.5">Image secondaire 2</label>
          <div class="file-upload-area" id="supp2Area" onclick="document.getElementById('supp2File').click()"><input type="file" name="image_supp_2" id="supp2File" accept="image/*" class="hidden" onchange="previewFile(this, 'supp2Preview', 'supp2Area')">
            <div id="supp2Preview" class="space-y-1"><i class="fas fa-cloud-upload-alt text-2xl text-[var(--text-muted)]"></i>
              <p class="text-xs text-[var(--text-muted)]">Optionnelle</p>
            </div>
          </div>
        </div>
      ...
      <div class="grid grid-cols-3 gap-3">
        <div><label class="block text-xs font-medium text-[var(--text-secondary)] mb-1.5">Date</label><input type="date" name="date_realisation" value="<?= date('Y-m-d') ?>" class="w-full input-glass px-3 py-2.5 text-sm"></div>
        <div><label class="block text-xs font-medium text-[var(--text-secondary)] mb-1.5">Client</label><input type="text" name="client_nom" class="w-full input-glass px-3 py-2.5 text-sm" placeholder="Nom"></div>
        <div><label class="block text-xs font-medium text-[var(--text-secondary)] mb-1.5">Ville</label><input type="text" name="client_ville" class="w-full input-glass px-3 py-2.5 text-sm" placeholder="Ville"></div>
      </div>
  ...
  <div id="editContent" class="modal-container fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[101] hidden w-[95%] max-w-lg p-6">
    <div class="flex items-center justify-between mb-5">
      <h3 class="text-lg font-bold text-[var(--text-primary)]"><i class="fas fa-edit mr-2 text-blue-500"></i>Modifier la réalisation</h3><button onclick="closeEditModal()" class="text-[var(--text-muted)] hover:text-[var(--text-primary)]"><i class="fas fa-times"></i></button>
    </div>
    <form method="POST" action="" enctype="multipart/form-data" class="space-y-4">
      <input type="hidden" name="realisation_id" id="editId">
      <div class="grid grid-cols-2 gap-3">
        <div><label class="block text-xs font-medium text-[var(--text-secondary)] mb-1.5">Titre *</label><input type="text" name="titre" id="editTitre" required class="w-full input-glass px-3 py-2.5 text-sm"></div>
        <div><label class="block text-xs font-medium text-[var(--text-secondary)] mb-1.5">Catégorie</label><select name="categorie" id="editCategorie" class="w-full input-glass px-3 py-2.5 text-sm">
            <option value="rideaux">Rideaux</option>
            <option value="voilages">Voilages</option>
            <option value="stores">Stores</option>
            <option value="installation">Installation</option>
      ...
      <div><label class="block text-xs font-medium text-[var(--text-secondary)] mb-1.5">Image principale</label>
        <div class="file-upload-area" id="editMainArea" onclick="document.getElementById('editMainFile').click()"><input type="file" name="image_principale" id="editMainFile" accept="image/*" class="hidden" onchange="previewFile(this, 'editMainPreview', 'editMainArea')">
          <div id="editMainPreview">
            <p class="text-xs text-[var(--text-muted)]">Image actuelle : <span id="editMainName" class="text-pink-500"></span></p>
            <p class="text-xs text-[var(--text-muted)]">Cliquez pour changer (laisser vide pour conserver)</p>
          </div>
        </div>
      </div>
      <div class="grid grid-cols-2 gap-3">
        <div><label class="block text-xs font-medium text-[var(--text-secondary)] mb-1.5">Image secondaire 1</label><input type="hidden" name="old_supp_1_url" id="oldSupp1Url"><input type="hidden" name="keep_supp_1" id="keepSupp1" value="0">
          <div class="file-upload-area" id="editSupp1Area" onclick="document.getElementById('editSupp1File').click()"><input type="file" name="image_supp_1" id="editSupp1File" accept="image/*" class="hidden" onchange="previewFile(this, 'editSupp1Preview', 'editSupp1Area'); document.getElementById('keepSupp1').value='0';">
            <div id="editSupp1Preview">
              <p class="text-xs text-[var(--text-muted)]">Aucune</p>
            </div>
          </div>
        </div>
        <div><label class="block text-xs font-medium text-[var(--text-secondary)] mb-1.5">Image secondaire 2</label><input type="hidden" name="old_supp_2_url" id="oldSupp2Url"><input type="hidden" name="keep_supp_2" id="keepSupp2" value="0">
          <div class="file-upload-area" id="editSupp2Area" onclick="document.getElementById('editSupp2File').click()"><input type="file" name="image_supp_2" id="editSupp2File" accept="image/*" class="hidden" onchange="previewFile(this, 'editSupp2Preview', 'editSupp2Area'); document.getElementById('keepSupp2').value='0';">
            <div id="editSupp2Preview">
              <p class="text-xs text-[var(--text-muted)]">Aucune</p>
            </div>
          </div>
        </div>
      ...
      <div class="grid grid-cols-3 gap-3">
        <div><label class="block text-xs font-medium text-[var(--text-secondary)] mb-1.5">Date</label><input type="date" name="date_realisation" id="editDate" class="w-full input-glass px-3 py-2.5 text-sm"></div>
        <div><label class="block text-xs font-medium text-[var(--text-secondary)] mb-1.5">Client</label><input type="text" name="client_nom" id="editClient" class="w-full input-glass px-3 py-2.5 text-sm"></div>
        <div><label class="block text-xs font-medium text-[var(--text-secondary)] mb-1.5">Ville</label><input type="text" name="client_ville" id="editVille" class="w-full input-glass px-3 py-2.5 text-sm"></div>
      </div>
  ...
  <div id="demandeContent" class="modal-container fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[101] hidden w-[95%] max-w-md p-6">
    <div class="flex items-center justify-between mb-5">
      <h3 class="text-lg font-bold text-[var(--text-primary)]">Détail demande</h3><button onclick="closeDemandeModal()" class="text-[var(--text-muted)] hover:text-[var(--text-primary)]"><i class="fas fa-times"></i></button>
    </div>
  ...
  <script>
    ...
    document.querySelectorAll('.tab-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        document.querySelectorAll('.tab-btn').forEach(b => {
          b.classList.remove('active', 'bg-gradient-to-r', 'from-pink-500', 'to-rose-500', 'text-white', 'shadow-md');
          b.classList.add('glass', 'text-[var(--text-secondary)]')
        });
        btn.classList.add('active', 'bg-gradient-to-r', 'from-pink-500', 'to-rose-500', 'text-white', 'shadow-md');
        btn.classList.remove('glass', 'text-[var(--text-secondary)]');
    ...
    function previewFile(input, previewId, areaId) {
      ...
      if (input.files && input.files[0]) {
        ...
        reader.onload = function(e) {
          preview.innerHTML = `<img src="${e.target.result}" class="preview-img" alt="Preview"><p class="text-xs text-emerald-500 mt-1">${input.files[0].name}</p>`;
    ...
    function openAjoutModal() {
      resetAjoutForm();
      openModal('ajoutOverlay', 'ajoutContent')
    }

    function closeAjoutModal() {
      closeModal('ajoutOverlay', 'ajoutContent')
    }
    ...
    async function openEditModal(id) {
      try {
        const resp = await fetch('?action=get_realisation&id=' + id);
        if (!resp.ok) {
    ...
    function closeEditModal() {
      closeModal('editOverlay', 'editContent')
    }
    ...
    function closePublishModal() {
      closeModal('publishOverlay', 'publishContent')
    }
    ...
    function closeDemandeModal() {
      closeModal('demandeOverlay', 'demandeContent')
    }

    ['ajoutOverlay', 'editOverlay', 'publishOverlay', 'deleteOverlay', 'demandeOverlay'].forEach(id => {
