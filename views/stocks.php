...
if (isset($_GET['action']) && $_GET['action'] == 'get_stock' && isset($_GET['id'])) {
    $query = $pdo->prepare("SELECT s.*, b.nom as boutique_nom, p.designation as produit_designation, p.umProduit FROM stock s JOIN boutiques b ON s.boutique_id=b.id JOIN produits p ON s.produit_matricule=p.matricule WHERE s.id=? AND s.statut=0");
...
try {
    ...
    $query = $pdo->prepare("SELECT s.*, b.nom as boutique_nom, p.designation as produit_designation, p.umProduit FROM stock s JOIN boutiques b ON s.boutique_id=b.id JOIN produits p ON s.produit_matricule=p.matricule WHERE s.statut=0 AND s.type_mouvement='approvisionnement' ORDER BY s.date_creation DESC LIMIT :limit OFFSET :offset");
...
<body class="h-screen flex overflow-hidden">
    ...
    <div id="stockContent" class="modal-container fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[101] hidden w-[95%] max-w-md p-6">
        ...
        <form method="POST" action="../models/traitement/stock-post.php" class="space-y-4">
            ...
            <div><label class="block text-xs font-medium text-[var(--text-secondary)] mb-1.5">Boutique *</label><select name="boutique_id" required class="w-full input-glass px-3 py-2.5 text-sm"><?php foreach ($boutiques as $b): ?><option value="<?= $b['id'] ?>"><?= htmlspecialchars($b['nom']) ?></option><?php endforeach; ?></select></div>
            <div>
                ...
                <select name="produit_matricule" id="produitSelect" required class="w-full input-glass px-3 py-2.5 text-sm" onchange="updateUnite()">
                    ...
                    <?php foreach ($produits as $p): ?>
                        <option value="<?= $p['matricule'] ?>" data-unite="<?= $p['umProduit'] ?>"><?= htmlspecialchars($p['designation']) ?> (<?= $p['matricule'] ?>)</option>
                    <?php endforeach; ?>
                </select>
                <div id="uniteInfo" class="hidden mt-2 p-3 rounded-xl bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800/30 text-xs"><span id="uniteText"></span></div>
            </div>
    ...
    <script>
        ...
        function updateUnite() {
            ...
            if (opt && opt.value) {
                ...
                document.getElementById('uniteHidden').value = opt.dataset.unite;
                document.getElementById('uniteText').textContent = `Type: ${opt.dataset.unite==='metres'?'Rideau':'Produit'} | Unité: ${u}`;
                document.getElementById('uniteInfo').classList.remove('hidden');
                document.getElementById('qteLabel').textContent = u;
                document.getElementById('seuilLabel').textContent = u;
            } else {
                document.getElementById('uniteInfo').classList.add('hidden')
            }
        }
        ...
        function openStockModal(id = null) {
            ...
            if (id) {
                ...
                fetch('stocks.php?action=get_stock&id=' + id).then(r => r.json()).then(d => {
                    if (d.success) {
                        document.querySelector('select[name="boutique_id"]').value = d.stock.boutique_id;
                        document.querySelector('select[name="produit_matricule"]').value = d.stock.produit_matricule;
